...
`scripts/generate-voices.py` prépare les 478 petits fichiers statiques avec `edge-tts` et FFmpeg, en générant uniquement ceux qui manquent. Les 36 mots viennent de `app/letterWords.ts` : conserver leur ordre pour préserver les voix et les étoiles déjà gagnées. Le service de synthèse n’est utilisé qu’à la préparation : aucun texte ni donnée de Lola n’est envoyé pendant le jeu. Le navigateur charge seulement le mot ou le calcul sélectionné.
